...
class cocotb {
  ...
  python_libraries(){
    ...
    this.str_out += "cocotb.result import TestFailure\nimport random\n";
  }
  clock_gen(){
    this.str_out += '\n@cocotb.coroutine\n';
  ...
  coco_test(){
    ...
    let x=0;
    while (((this.str.ports[x]["direction"] !== "in" && this.str.ports[x]["direction"] !== "input") 
            && (this.str.ports[x]["type"] !== "std_logic" && this.str.ports[x]["type"] 
            !== "")) && x<this.str.ports.length  ) {
      x++;
    ...
    for (let i = 0; i < this.str.ports.length-1; i++) {
      if ((this.str.ports[i]["direction"] === "in" || this.str.ports[i]["direction"] === "input") && i !==x) {
    ...
    x = 0;
    while ((this.str.ports[x]["direction"] !== "out" && this.str.ports[x]["direction"] 
      !== "output") && x<this.str.ports.length-1) {
      x++;
    }
    this.str_out += '    if int(dut.' + this.str.ports[x]["name"] + ') == int(dut.' 
