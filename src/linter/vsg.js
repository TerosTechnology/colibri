...
class Vsg {
    ...
    async lint_from_file(file, options) {
        ...
        let output_junit = await this._vsg_exec(file, temp_file_junit, options);
    ...
    async lint_from_code(code, options) {
        ...
        let temp_file_junit = await this._create_temp_file_of_code("");
        let output_junit = await this._vsg_exec(temp_file_code, temp_file_junit, options);
        let errors = await this._parse_junit(output_junit);
        return errors;
    }
    ...
    async _vsg_exec(code_file, junit_file, options) {
        let args = '';
        if (options !== undefined && options.custom_arguments !== undefined) {
            args = options.custom_arguments;
        }
        let code_file_normalized = code_file.replace(' ', '\\ ');
        let json_file_file_normalized = junit_file.replace(' ', '\\ ');
        let command = `vsg -f ${code_file_normalized} --all_phases ${args} --js ${json_file_file_normalized}`;
